...
Core tests take longer than any other Liberty tests, due to the high amount of computational work involved in validating core components.
...
To run only Liberty's core tests (after building):
...
To run only Liberty's crypto tests (after building):
...
```bash
libertyd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `liberty-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Liberty's hash tests (after building):
...
To run only Liberty's performance tests (after building):
...
To run only Liberty's unit tests (after building):
