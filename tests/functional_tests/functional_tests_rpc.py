...
# a main offline libertyd, does most of the tests
# a restricted RPC libertyd setup with RPC payment
# two local online libertyds connected to each other
N_LIBERTYDS = 4

# 4 wallets connected to the main offline libertyd
# a wallet connected to the first local online libertyd
N_WALLETS = 5
...
libertyd_base = [builddir + "/bin/libertyd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "libertyd_p2p_port", "--rpc-bind-port", "libertyd_rpc_port", "--zmq-rpc-bind-port", "libertyd_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "libertyd_data_dir", "--log-level", "1"]
libertyd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/liberty-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_LIBERTYDS):
  command_lines.append([str(18180+i) if x == "libertyd_rpc_port" else str(18280+i) if x == "libertyd_p2p_port" else str(18380+i) if x == "libertyd_zmq_port" else builddir + "/functional-tests-directory/libertyd" + str(i) if x == "libertyd_data_dir" else x for x in libertyd_base])
  if i < len(libertyd_extra):
    command_lines[-1] += libertyd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/libertyd' + str(i) + '.log', 'a+'))
